...
下面这些站点页把框架带向实践。它们不提出新理论，而是把工作稿分别应用到人类能力和开源实现两个方向。
...
这套框架是为检验而设计的，不只是用来论证。下面是它提出的、仍然敞开的实证问题：
...
这些工作稿目前更像研究框架与开源宣言，而不是已经封闭完成的理论。目标是提出一套可讨论、可实现、可验证的语言：用来描述 LLM 什么时候会平庸，什么时候会卓越，以及人和系统应该怎样把局部能力转化为稳定的任务价值。
