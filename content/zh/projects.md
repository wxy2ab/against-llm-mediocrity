...
项目页承接论文页的实证问题：如果知识治理不只是一个解释框架，它就必须能落到可保存、可检查、可撤销的软件对象上。案例库当前给出的方向是：先把控制空间做成对象，再把分层路由、连续性审计、pairwise 判断和平台期判断做成可观察的治理过程。这里的项目不追求一次性做出“大而全 agent”，而是拆出最小可验证组件。
