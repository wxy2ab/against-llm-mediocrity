...
这一页记录的是这项工作的软件侧：怎样把论文里关于知识治理的想法，做成能运行、能检查、也能撤销的对象。目前已经发布一个项目，其余还处在规划阶段。下文提到的 GKO，指受治理知识对象（Governed Knowledge Object / GKO），也就是一个被存下来的判断单元，可以被检索、弱化或撤销。
...
[sgar](https://github.com/wxy2ab/sgar) 是一个 embedded coding agent，用来把自动化修复、自动化运维和长程代码编辑能力嵌入到系统里。它同时提供 standalone CLI、可嵌入的 agent runtime，以及面向 OpenClaw 的长程代码编辑技能。

这个项目的重点，不是“调一次 LLM 让它写点代码”，而是把代码编辑、状态治理、阶段推进、审计验证和运行痕迹拼成一个能长期运行的 agent 模型。它的设计基于 [审计工程](https://github.com/wxy2ab/against-llm-mediocrity/blob/main/docs/audit-engineering.zh-CN.md) 与 [状态治理智能体范式（State-Governed Agent Regime / SGAR）](https://github.com/wxy2ab/against-llm-mediocrity/blob/main/docs/state-governed-agent-regime.zh-CN.md)。更具体一点说，就是把硬状态、action/delta、trace、验证记录和 `.sgar/` 工作区外部化。这样做的直接收益，是让长程运行里常见的漂移、跳步和“其实没做完却像做完了”更容易被压住。
...
下面这些方向，都是从当前工作稿顺着推出来的实现路线，不是额外加上的理论主张。它们要验证的是：受治理知识、硬状态转移、升级协议和失配诊断，能不能真的长成有用的软件对象。

每一个方向都接着论文里的同一个实证问题往下走：**如果知识治理不只是解释框架，它就得落到可保存、可检查、可撤销的软件对象上。** `sgar` 已经先把硬状态、阶段推进、验证和 trace 做成了可运行对象。后续项目会继续沿这个方向，把控制空间、分层路由、连续性审计、pairwise 判断和平台期判断也变成可观察的治理过程。目标不是一次做出“大而全 agent”，而是把每个方向都拆成最小可验证组件。
...
面向需要自主推进但不能越权行动的 agent 工作流。输入是当前任务状态、阻塞变量、风险等级和可逆工作；输出是最小充分人类问题（MSHQ）或受治理升级对象（GEsO）。
