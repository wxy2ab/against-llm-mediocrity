  ...
  regimes: {
    ...
    items: {
      mediocre: {
        ...
        tag: "概率峰偏离价值峰",
        desc:
          "预算内能到达的候选集中在流畅、合理、可局部改进、却远离高价值解的区域。继续采样或润色提高平均质量，仍碰不到决定性结构。",
      ...
      local: {
        ...
        tag: "最常见的实际状态",
        desc:
          "模型的局部续写倾向与任务价值的一部分对齐：能压缩、改写、列举、比较、构造提纲。但全局成功仍依赖隐藏状态、长程依赖、真实目标或验证。",
      ...
      extraordinary: {
        ...
        tag: "概率与价值同向",
        desc:
          "高价值输出不再是低概率尾部事件，而是容易到达的区域。当任务结构、表示与成功标准都适合模型时，每个自然的下一步都把整体推向更好的结果。",
