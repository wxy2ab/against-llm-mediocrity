...
- Latent-Conditioned Aggregation mismatch: [English](docs/latent-conditioned-aggregation-mismatch.md) · [中文](docs/latent-conditioned-aggregation-mismatch.zh-CN.md) — An advanced aggregation-mismatch subtype where multiple surface choices are jointly constrained by one complete latent structure that must remain complete at the control layer but not fully explicit in delivery.
