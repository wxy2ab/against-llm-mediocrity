...
这个站点给出的答案很明确：**有些问题确实属于 LLM 的结构性天花板，不能指望仅靠继续训练自动消失。** 更有意义的工作，是识别这些天花板是什么，以及如何用工程手段尽可能绕开、约束、补偿或重构它们。
...
- 哪些失败不是暂时能力不足，而是结构性天花板在起作用；
- 如何把局部对齐转化为可验证、可治理、可复用的工程修复；
- 当结构性天花板无法被训练直接抹平时，agent 与人的协作范式应当如何重构。
