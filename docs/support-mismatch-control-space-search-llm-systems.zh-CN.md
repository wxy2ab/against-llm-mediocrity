...
- **观测-表征失配**——可行通道本可提供的决定性信息没有进入系统表征。
- **拟合边界失配**——正确能力没有激活。
- **聚合失配**——局部好组件无法组合成全局价值。
- **规格失配**——评价器优化了错误代理目标。
...
观测-表征失配发生在可行通道本可取得或保留的决定性信息没有进入操作表征 `Z`。
...
状态失配发生在固定 `Z` 下，系统形成或更新的状态信念偏离同一证据支持的信念，并改变行动排序。不确定性本身不构成失配。

即便状态信念正确，支持失配仍可能发生。系统知道任务体制，却仍无法生成该体制所需的稀有结构。
