...
这是一篇技术工作稿。它的核心论点是：许多高价值 LLM（Large Language Model，大语言模型）系统之所以失败，是因为任务高价值所需的结构在已部署搜索流程下根本不可达——它们没有被采样、没有被保存、没有被成熟、也没有被识别。本文把这种失败命名为**支持失配**，将它与其他原始失配类型加以对比，并围绕**控制空间搜索**发展出一套治理方法。
...
支持失配是可达性失败：高价值结构位于系统候选空间中的低概率、低覆盖、低识别或早期剪枝区域。它不同于观测-表征失配，后者是决定性变量没有进入系统表征；不同于拟合边界失配，后者是正确能力没有激活；不同于聚合失配，后者是局部好组件无法组合成全局价值；也不同于规格失配，后者是评价器优化了错误代理目标。在支持失配中，高价值结构原则上可表达且有价值，但系统的策略、搜索算子、先验或预算使它不太可能作为活候选出现。

中心干预是 **控制空间搜索**。受治理系统不是反复采样最终输出，而是在更接近任务价值因果决定因素的中间结构上搜索：schema、join path、依赖图、状态假设、约束、证明义务、工具计划、不变量集合、候选机制或修复增量。随后由 renderer 把这些受治理控制对象映射为最终产物。这种转换可以通过更常见、可检查、可组合的中间对象，让稀有最终输出变得可达，从而提高有效支持。
