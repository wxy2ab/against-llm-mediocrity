# 从聚合失配 V1–V12、V14–V16 到 Agent 工程：完整经验总结

**证据截止：** 2026-07-30

**定位：** 将受控实验转化为可实现、可度量、可撤销的 Agent 架构原则。

**English:** [From Aggregation Mismatch V1–V12 and V14–V16 to Agent Engineering](./aggregation-mismatch-agent-engineering-lessons-v1-v12-v14-v16.md)

**配套证据：** [V1–V12、V14–V16 实验总览](./aggregation-mismatch-v1-v12-v14-v16-experiment-summary.zh-CN.md)
...
11. 在 unlock 权限与 \(S_2\) 匹配后，receipt-only、旧状态、target slice 与结构化
    rebase plan 的独立信息效应分别是多少？
12. V16 的 same-state retry 结果能否在第二模型和真实过期 lease、Git lock、数据库
    lock 中复现？
