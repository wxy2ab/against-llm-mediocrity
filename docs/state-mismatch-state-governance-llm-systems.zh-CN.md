...
主结构理论把 LLM 系统失败定义为从世界到输出管线中的价值丢失。对象模型规范定义受治理知识对象（GKO）、审计发现、控制增量、回归护栏和状态记录等受治理对象。审计工程解释失败如何被定位并写回控制空间。状态治理型 Agent 体制（State-Governed Agent Regime，SGAR）解释已验证进展如何成为已提交硬状态。各个失配文档则展开每个管线站点。
