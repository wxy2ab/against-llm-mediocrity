...
- [1. 问题：把状态当成已知来行动](#1-问题把状态当成已知来行动)
- [2. 形式化定义](#2-形式化定义)
- [3. 为什么状态失配是原始失配](#3-为什么状态失配是原始失配)
- [4. 价值保存管线中的状态站点](#4-价值保存管线中的状态站点)
- [5. 状态失配分类](#5-状态失配分类)
- [6. 诊断特征](#6-诊断特征)
- [7. 状态治理](#7-状态治理)
- [8. 核心状态治理对象](#8-核心状态治理对象)
- [9. 诊断工作流](#9-诊断工作流)
- [10. 面向状态失配的审计工程](#10-面向状态失配的审计工程)
- [11. 面向状态的知识治理](#11-面向状态的知识治理)
- [12. 与 SGAR 集成](#12-与-sgar-集成)
- [13. Text-to-SQL 中的状态失配](#13-text-to-sql-中的状态失配)
- [14. 代码与调试中的状态失配](#14-代码与调试中的状态失配)
- [15. 研究、策略与咨询工作中的状态失配](#15-研究策略与咨询工作中的状态失配)
- [16. 与其他失配的复合交互](#16-与其他失配的复合交互)
- [17. 状态治理型渲染](#17-状态治理型渲染)
- [18. 与形式传统的关系](#18-与形式传统的关系)
- [19. 什么时候不需要状态治理](#19-什么时候不需要状态治理)
- [20. 状态失配主张的自审计](#20-状态失配主张的自审计)
- [21. 状态失配检查清单](#21-状态失配检查清单)
- [22. 结论](#22-结论)
- [Appendix A: 最小对照例](#appendix-a-最小对照例)
- [Appendix B: 状态治理对象目录](#appendix-b-状态治理对象目录)
- [Appendix C: 状态控制增量类型](#appendix-c-状态控制增量类型)
- [Appendix D: 紧凑定义](#appendix-d-紧凑定义)

---

## 摘要

高价值 LLM 系统经常失败，并不是因为模型缺少流畅语言、相关知识或局部上合理的策略，而是因为系统在决定性潜在状态尚未解决时，就像已经知道自己处在哪种情境里一样行动。同一个 prompt、schema、用户指令、代码 trace、研究问题、市场描述或工具输出，可能同时兼容多个任务状态，而最优行动在这些状态之间可能完全反转。在这类情况下，更自信的生成反而会让表现变差：模型把不确定性塌缩成一个方便的解释，然后优化一个不适用于当前状态的策略。
...
主结构理论把 LLM 系统失败定义为从世界到输出管线中的价值丢失。对象模型规范定义受治理知识对象（GKO）、审计发现、控制增量、回归护栏和状态记录等受治理对象。审计工程解释失败如何被定位并写回控制空间。状态治理智能体范式（State-Governed Agent Regime，SGAR）解释已验证进展如何成为已提交硬状态。各个失配文档则展开每个管线站点。
...
它发生在系统已经拥有操作表征 `Z` 之后、但尚未能够正确路由能力、搜索候选、组合输出或评价成功之前。系统不仅要知道哪些信息可用，还必须知道这些信息指向什么情境。
...
规格失配关注目标代理是否匹配真实效用。状态失配关注哪个条件或区间决定了目标。

系统可能为每个状态都有正确的 rubric，却无法识别哪个 rubric 适用。反过来，它也可能正确识别了状态，却使用了糟糕的代理目标。
...
拟合边界失配关注某个已学习能力是否在真实适用域内被激活，或在真实适用域外被激活。状态失配关注哪个状态应当支配能力选择。

二者经常复合。错误的状态假设可能触发错误的能力。但修复目标不同：
...
支持失配关注高价值候选是否可达。状态失配关注系统是否知道应该偏好哪个候选家族。

正确候选可能可达，却因为系统假设了错误状态而被拒绝。或者系统可能正确识别了状态，却无法生成该状态下所需的低支持结构。

### 3.5 不是聚合失配

聚合失配关注局部部分能否组合成全局价值。状态失配关注系统是否在正确的潜在状态假设下组合。

一个计划可能在状态 `h1` 下内部一致，但对状态 `h2` 是错误的。它的失败不只是局部到全局的组合问题；它是状态条件化误用。
