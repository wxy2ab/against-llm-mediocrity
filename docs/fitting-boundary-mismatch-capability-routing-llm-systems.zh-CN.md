...
——一个定性说明与可证伪假设，尚不是受控的机制证明。

---

## 摘要

大型语言模型经常拥有某些能力，却不能在真正需要这些能力的情境中稳定表达。反过来，它们也可能在并不适用的情境中激活已经学到的行为、专家腔调、解题模板、拒答策略、安全仪式或 benchmark 模式。

本文把 **拟合边界失配** 展开为受治理 LLM 系统中的一种原始失败模式：能力真实适用域与模型/系统实际激活该能力的域之间发生错位。
...
LLM 系统可能具备完成任务所需的知识、模式、技能、工具或推理例程，却仍然失败，因为隐式路由器没有激活它。反过来，系统也可能因为表面证据类似训练期或 prompt 诱导的触发条件而激活某种能力，即使任务并不真正满足该能力的适用条件。

这类失败不能还原为知识缺失、概率支持不足、规格歧义、状态不确定或聚合失败。它们发生在世界到输出管线中的 **能力路由站点**。
...
随后本文发展一套能力路由的实践理论，涵盖能力画像、触发证据、抑制器、角色吸引子、边界扰动、路由增量、路由 GKO（Governed Knowledge Object，受治理知识对象）和边界回归护栏。它说明拟合边界失配如何与知识治理、审计工程和状态治理智能体范式（State-Governed Agent Regime，SGAR）相互作用。

目标是把能力激活从 prompt 表面形式的隐式副作用，转化为可审计、可修订、状态感知的控制层。
...
本文按照“定义—诊断—修复”的顺序展开。第 1–4 节确立拟合边界失配是什么，以及它为何与众不同。第 5–10 节描述路由管线，并说明如何诊断边界失败。第 11–21 节讲述路由治理及其设计模式。第 22–26 节走完几个具体示例，第 27–32 节以形式化主张、成本与实用清单收尾。

- [1. 引言](#1-引言)
- [2. 在价值保存结构理论中的位置](#2-在价值保存结构理论中的位置)
- [3. 核心定义](#3-核心定义)
- [4. 能力不等于行为](#4-能力不等于行为)
- [5. 能力路由管线](#5-能力路由管线)
- [6. 触发证据、抑制器与吸引子](#6-触发证据抑制器与吸引子)
- [7. 失败形态](#7-失败形态)
- [8. 与其他失配的区分](#8-与其他失配的区分)
- [9. 为什么拟合边界失配对 LLM 平庸重要](#9-为什么拟合边界失配对-llm-平庸重要)
- [10. 诊断方法](#10-诊断方法)
- [11. 路由治理](#11-路由治理)
- [12. 路由 GKO](#12-路由-gko)
...
- [19. 与观测-表征失配的相互作用](#19-与观测-表征失配的相互作用)
- [20. 与聚合失配的相互作用](#20-与聚合失配的相互作用)
- [21. 能力路由设计模式](#21-能力路由设计模式)
- [22. 示例：Text-to-SQL](#22-示例text-to-sql)
- [23. 示例：代码修复](#23-示例代码修复)
- [24. 示例：量化研究与策略设计](#24-示例量化研究与策略设计)
- [25. 示例：安全与拒答](#25-示例安全与拒答)
- [26. 示例：研究与理论写作](#26-示例研究与理论写作)
- [27. 能力路由作为控制空间搜索](#27-能力路由作为控制空间搜索)
- [28. 形式化主张](#28-形式化主张)
- [29. 路由治理的成本与风险](#29-路由治理的成本与风险)
- [30. 拟合边界主张的撤销触发器](#30-拟合边界主张的撤销触发器)
- [31. 实用清单](#31-实用清单)
- [32. 结论](#32-结论)
...
一种常见的 LLM 失败描述是：模型“不知道”某件事，或“不会”某种推理。这种描述往往太粗。

许多任务中，模型在被直接要求、换一个 prompt 框架、给一个最小例子、展示反例或经由专门流程路由时，能够表现出相关能力。但在原始任务语境里，这个能力没有被激活。模型做了别的事：套用通用模板，进入无关专家 persona，过度应用安全模式，写出合理套话，做浅层类比，或生成一个流畅但绕过真实操作的答案。
...
本文称这种失败模式为 **拟合边界失配**。一个能力有真实适用域：使用它会提升任务价值的情境集合。模型或系统也有激活域：实际触发该能力的情境集合。

当二者偏离时，即便能力存在、相关信息存在、正确答案有足够支持、目标大致明确，系统仍可能失败。
...
要理解这种路由失败发生在何处，我们需要把它置于更广泛的价值保存管线之中。
...
这个位置很重要。管线中的每个站点都有对应的失败模式：

- 如果可行通道本可提供的决定性信息从未进入 `Z`，失败是观测-表征失配。
- 如果固定 `Z` 后，系统信念偏离证据支持信念并改变行动排序，失败是状态失配；若正确保留不可约不确定性，则没有系统失配。
- 如果能力适用却未被激活，或能力不适用却被激活，失败是拟合边界失配。
- 如果能力已激活但正确结构在策略下概率很低，失败是支持失配。
- 如果局部能力输出无法组合，失败是聚合失配。
- 如果评价器奖励错误东西，失败是规格失配。

因此，拟合边界失配不是分类法的可选补丁。它对应管线中一个结构上独立的站点。

明确了这一位置后，我们现在给出形式化定义。
...
要理解为什么选错了模式，我们需要考察激活能力的证据是什么，以及什么会抑制它。
...
二者会相互作用。错误或过期信念可能触发错误能力。但修复目标不同：状态失配需要修复信念形成、更新或使用；拟合边界失配需要更好能力路由。若缺的是本可取得的信息，则上游诊断是观测-表征失配。
...
为什么这在实践中重要？因为拟合边界失配是所谓“LLM 平庸”现象的主要驱动因素之一。
...
一旦诊断出拟合边界失配，修复方式不是写更好的 prompt，而是显式治理路由器本身。
...
在长程系统中，这些护栏和路由决策还必须作为已提交的状态持续存在。
...
路由并非决定能力是否成功的唯一因素。支持度也很重要，二者相互耦合。
...
即便有了正确的路由和表征，系统仍需把多个能力协调成连贯的整体。
...
这些示例揭示了一个更广泛的原理：路由本身可以被视为一个搜索问题。
