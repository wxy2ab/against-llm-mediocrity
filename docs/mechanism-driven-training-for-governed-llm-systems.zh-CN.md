...
当前的受治理 LLM 理论栈，主要描述的是 **运行时治理**。知识治理、审计工程、对象级控制增量、回归护栏，以及状态治理智能体范式（SGAR），都围绕一个冻结模型运行。它们通过改变任务表征、控制对象、审计回路、路由规则、执行过程与已提交状态来修复失败——而不改变模型权重。
...
本文定义 **机制驱动训练**（Mechanism-Driven Training）：它是运行时治理在训练侧的对应层。其核心主张很明确：同一套用于修复定位的八轴机制诊断，也应当驱动训练干预。关键差异不在诊断本身，而在 **修复层**。
...
两者的对比很鲜明：运行时治理处理局部、可逆、任务特定的失败；机制驱动训练则把那些反复出现、跨任务、由学习组件引起的失败，从缺陷台账提升为训练课程、边界数据、grounding 数据、奖励纠偏，或能力支持数据。
...
机制驱动训练不会新增一种原始失配，也不会取代六失配分类法。相反，它占据的是诊断-机制桥中的训练侧分支。
...
基于这一分解，我们可以区分部署时已固定的组件与可以被学习的组件。`\Omega_{sys}` 属于 `M_θ`，而不是 `E`：`Ω` 与 `O` 描述环境原则上能提供哪些观测，`\Omega_{sys}` 则描述已部署系统实际暴露了哪些观测通道。
...
由于每条混合轴都横跨训练与运行时，选择正确的修复层取决于失败的性质。
...
因为症状可能掩盖真正的原因，机制驱动训练要求把每一次失败都沿特定的诊断链追溯到底。因此，机制驱动训练遵循如下纪律：
...
面对这三种路径，默认选择应是局部修复。机制驱动训练不应成为第一反应。默认应该是：
...
机制驱动训练把审计工程从运行时写回，扩展到训练侧写回。这一闭环确保发现不会停留在台账中而无动作。
...
由于副作用不可避免，每一次机制驱动训练干预都需要显式的边界治理。
...
机制驱动训练应当是有选择性的。在投入训练之前，请先确认该缺陷确实属于模型内部，而非运行时层。
...
这些都属于 Agent 层治理机制。当这些运行时补丁不足以解决问题时，同样的发现就可以提升到训练层。
...
成功标准不只是通用 benchmark 分数提升。真正目标是：在不造成不可接受边界损伤的前提下，降低反复出现的学习组件缺陷。要判断训练干预是否成功，应使用针对机制的指标，而不仅仅是 benchmark。
...
机制驱动训练本身也可能失败。即使诊断正确，训练过程也可能以几种典型方式出错。
