...
**English:** [From Aggregation Mismatch V1–V12 and V14–V17 to Agent Engineering](./aggregation-mismatch-agent-engineering-lessons-v1-v12-v14-v17.md)

**配套证据：** [V1–V12、V14–V17 实验总览](./aggregation-mismatch-v1-v12-v14-v17-experiment-summary.zh-CN.md)
...
14. Retry 必须依赖权威状态迁移。lock、version、authority 与 precondition 未改变时，
    应等待或升级，而不是扩大上下文后重复相同写入。
15. 有效状态迁移后，先发送最小充分 receipt 与 verified plan；只有操作语义依赖当前值
    时才升级到 target slice 或完整状态。
16. 把 typed escalation 作为一等安全终态，但不能计入 commit 成功或下游问题解决。
...
- V17 四个信息臂均为 24/24：对信息 superiority/等价属于 U；对 receipt-only 足以
  完成本绝对 plan 任务属于 E。
- V17 typed escalation 24/24 且 commit=0：对冻结治理合同属于 E，不证明升级提高
  任务完成率。
...
Rebase 也各使用 48 turns，而单 turn 臂为 24。V16 把三个 conflict 臂都匹配为两个
turn；Reread 中位 29,589 tokens 仍为 0/24，runtime Rebase 中位 17,439 为 24/24。
不能把 timeout usage
...
V16 的 96 episodes、1,752 events、168 provider turns 同样零错配，三个冲突臂的
第二 turn 都保留在原 episode。
V17 的 192 episodes、3,840 events、384 provider turns 同样可重建，每个正式
episode 恰好 20 条事件。账本还证明 Module A 每个 task 的四个信息臂都提交了相同
recovery operations，把“模型实际使用的上下文”与 headline success 分开。
...
- V15 机器主检验通过，但冻结 Pilot 文字与可执行门不一致，证据状态为
  `share_with_caveats`。
- V16 机器主检验通过，但冻结 manifest 残留陈旧 Pilot `stop_if`，证据状态同样为
  `share_with_caveats`。
- V17-A1 在 ceiling 下未通过 superiority 门，不能改写成等价；V17-B1 通过，但比较
  escalation compliance 与 commit completion，因此结果必须保留 endpoint 标签。
...
V16 匹配三个 conflict 臂的 provider turn 数。Generic Retry 与 Reread Only 仍锁定，
均为 0/24；runtime Unlock + Rebase 为 24/24。生产规则是：
...
V16 识别的是 runtime 政策包，不是纯信息效应：只有 Rebase 得到恢复权限、解锁
\(S_2\) 和 target slice。不能据此省略有效状态迁移后的必要上下文。

V17 固定 unlock 和权威 \(S_2\)。对于 verified absolute-value plan，RECEIPT、
OLDSTATE、TARGET-SLICE、REBASE-PLAN 均为 24/24，并在每个 task 发出相同
operations；OLDSTATE 的中位 token 比 RECEIPT 高 74.9%。这支持**本操作类型的最小
充分上下文梯度**，不支持总体等价：

```text
typed receipt + verified plan
→ 当前值会影响操作时加入 target-state slice
→ 只有 verifier/compiler 请求时才扩大到更广状态
```

当 unlock 不可行时，V17 的 typed Escalate 为 24/24 合法终态、0/24 commit。工程
规则是把 `ESCALATED_FOR_POLICY` 做成有审计和下游 owner 的一等终态，并与
`COMMITTED`、`problem_resolved` 分开统计。
...
Context Broker
  → receipt + verified plan → target slice → 按需扩大状态
...
8. commit、reject、wait、escalate、rollback 分离终态。
...
9. 只有操作语义或 verifier 诊断要求时才扩大恢复证据的 context broker。
...
8. escalation quality、下游解决率、排队时间和人工成本与 commit success 分开。

## 18. 应用映射

- **代码 Agent：** symbol/AST plan → compile current edits → seal → hash recheck →
  format/type/test → diff audit → atomic commit；symbol 被其他 writer 占用时 wait 或
  typed escalate，不重复 apply。
...
- **多 Agent：** 只分发 ready task，共享 hard ledger；每个 sealed plan 绑定 base
  ...
  last-writer-wins；每次 escalation 指定 conflict owner 与下游 resolver。
...
- lock/version 未改变时，只扩大对话上下文并重复同一写入；
- recovery 被计作新独立样本；
- 只报成功率，不报预算、tail 和 commit risk；
- 把 48/48、768/768 写成生产 100%；
- 把 failed gate 写成无效，把 ceiling 写成等价；
- 把 escalation compliance 当作 commit success 或业务问题已解决；
- 未验证最小上下文是否足够，就在每次恢复中倾倒完整旧状态；
- 根据探索性子组硬编码 router；
- 把单模型合成结果外推为普遍定律；
...
11. 在非 ceiling 的相对、CAS 和跨对象操作中，target-state slice 何时优于同 unlock
    下的 receipt-only？
12. 对 absolute plan，能否用预注册 cost–reliability 门证明 receipt-only
    non-inferior？
13. 同时开放 commit、wait、rebase、escalate 时，Agent 能否自主识别并路由冲突，
    而不是依赖接口预设？
14. typed escalation 最终有多少被解决，人工成本和等待时间是多少？
15. V16 的 same-state retry 结果能否在第二模型和真实过期 lease、Git lock、数据库
    lock 中复现？
...
> 权威状态；stale/locked reject 走有界 conflict governor；恢复上下文按需披露；typed
> escalation 是有审计的 non-commit 终态；所有写入经过 atomic executor、global
