...
`V_Z` 与 `V_feas` 都在历史依赖策略上优化；静态任务可令 `t=1`。直接访问世界状态的理想值仍可写为 `V_S = max_(π:S_≤t→A) E_s[U(π(S_≤t),s_t)]`，但 `V_feas < V_S` 的部分是任务信息结构造成的不可约部分可观测性，不应算作观测-表征失配。观测-表征失配只计算当前系统相对于可行最佳通道的损失 `V_feas - V_Z`。
