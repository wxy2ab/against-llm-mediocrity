...
LLM 系统越来越多地围绕迭代生成、批判、检索、工具使用、执行反馈和修订来构建。这些循环常常能提升表面质量，但它们不会自动产生持久的系统学习。

模型可能批判一个答案、生成更好的候选，却仍然没有触及底层失败模式。下一个类似任务可能再次失败，因为缺陷从未被定位、写回控制空间、加上回归护栏，或提交进状态。

本文提出 **审计工程**：一种把失败转换为受治理 LLM 系统中持久控制改进的纪律。当前最稳定的压缩口径可以概括为四个方面：它以**生成-验证不对称**为基础原理；优先利用**环境反馈、编译器、测试、runtime、执行结果**等较硬的证据通道；在文本、claim、design doc 等任务上可操作化为 **claim-evidence 驱动的结构化对抗审计**；并可由独立的 **Auditor / Repair Router / Regression Auditor** 等角色实现。

这四个方面并非完全同级：第一个是第一原理，第二个是高价值证据来源，第三个是特定任务中的操作范式，第四个则是组织实现层。它们的共同作用，是把审计从“事后评论”升级为可复核、可回写、可治理的工程机制；而在更高一层，系统仍需通过 **oracle classification / engine routing** 决定何时走 audit、何时走 SGAR、何时先硬化 gate，或何时诚实 No-Go。

审计不是分数、偏好判断或通用批评。有用的审计会识别局部缺陷，用证据支撑它，把它映射到失配类型，并识别真正要改变的任务特定控制对象。它还会在已可操作化时记录机制归因，产生控制增量，并创建一个在该缺陷家族复发时会失败的回归护栏。

审计工程建立在两个不对称性之上。第一，生成卓越产物往往比识别具体缺陷更难。第二，提前写出完整规格往往比通过反例修复规格更难。

高价值系统应利用这些不对称性，把候选失败转换为任务表征、能力路由、支持搜索、聚合约束、规格、验证和硬状态中的改进。
...
- [3. 审计不是什么](#3-审计不是什么)
- [4. 基础不对称性](#4-基础不对称性)
- [5. 价值保存管线中的审计](#5-价值保存管线中的审计)
- [6. 审计发现](#6-审计发现)
- [7. 控制增量](#7-控制增量)
- [8. 审计生命周期](#8-审计生命周期)
- [9. 失配特定审计模式](#9-失配特定审计模式)
- [10. 失败定位](#10-失败定位)
- [11. 回归治理](#11-回归治理)
- [12. 缺陷账本](#12-缺陷账本)
- [13. 验证器权威与验证器完整性](#13-验证器权威与验证器完整性)
- [14. 审计模式](#14-审计模式)
- [15. 反模式](#15-反模式)
- [16. 与知识治理的集成](#16-与知识治理的集成)
- [17. 与 SGAR 的集成](#17-与-sgar-的集成)
- [18. 风险分层审计强度](#18-风险分层审计强度)
- [19. 参考示例：Text-to-SQL](#19-参考示例text-to-sql)
- [20. 参考示例：代码修复](#20-参考示例代码修复)
- [21. 审计关闭标准](#21-审计关闭标准)
- [22. 审计之审计](#22-审计之审计)
- [23. 与形式传统的关系](#23-与形式传统的关系)
- [24. 最小可行审计工程](#24-最小可行审计工程)
- [25. 结论](#25-结论)
...
在本文里，审计工程不会被压缩成单一技巧或单一 agent。更准确地说，它同时涉及：以生成-验证不对称为基础的失败利用；以环境反馈和执行证据为代表的高保真信号通道；以 claim-evidence 为代表的结构化审计操作；以及以 Auditor、Repair Router、Regression Auditor 为代表的实现分工。后文会分别展开这些方面，但不把它们误当成完全同级的理论层。
...
因此，审计工程把失败视为资源。失败揭示了任务、系统、规格、表征、验证器或状态转移契约中的信息。高价值审计尤其应尽可能绑定到可复核证据，例如编译错误、测试失败、runtime trace、执行结果、类型检查、结构化 claim-evidence 约束与其他可重复观察的 failure witness，而不是停留在模糊评价。

目标是把这些信息捕获为系统以后能使用的形式。
