...
Audit Engineering，中文可称为**审计工程**，是一种面向 LLM 与 Agent 系统的推理时工程范式。它不把高质量产物主要寄托于一次性写出完美提示词，也不把审计理解为生成之后的简单打分。相反，它把审计循环设计成一套完整机制：发现真实目标、定位失配、回写控制对象、保留历史轨迹、在必要时更新硬状态，并防止下一轮退化。
