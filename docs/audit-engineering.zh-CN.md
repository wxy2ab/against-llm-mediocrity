...
Audit Engineering，中文可称为**审计工程**，是一种面向 LLM 与 Agent 系统的推理时工程范式。它不把高质量产物主要寄托于一次性写出完美提示词，也不把审计理解为生成后的简单评分；它把审计循环设计成一套发现真实目标、定位失配、回写控制对象、保留历史轨迹并防止下一轮退化的机制。

它的基本判断是：在大量开放式、高失配、隐含标准强的任务中，直接生成优质产物很难，但识别产物哪里不对、哪里不完整、哪里偏离真实目标，往往相对容易。审计工程把这种**生成—验证不对称**工程化：

> 让生成暴露问题，让审计把问题转化为可执行的控制增量，再让 Agent 沿着这些增量继续迭代。

这使它有资格与 Prompt Engineering、Context Engineering 和 Hardness Engineering 并列：Prompt Engineering 控制“怎么问”，Context Engineering 控制“给什么信息”，Hardness Engineering 控制“任务、环境与验收边界有多硬”，Audit Engineering 则控制“生成后如何发现真实目标、定位失败、回写控制空间，并防止下一轮退化”。

---

## 1. 为什么需要审计工程

传统 Prompt Engineering 隐含了一个前提：用户能够在任务开始前，把目标、约束、风格、成功标准和边界条件尽可能完整地写入提示词。但在许多高价值任务中，这个前提并不成立。用户可能知道“不对”，却说不清“怎样才对”；知道结果没有达到目标，却无法在第一轮生成前完整定义那个目标。

这不是单纯的表达能力问题，而是任务本身具有以下特征：

1. **真实目标会在生成中涌现**：只有看到候选产物后，用户或审计器才知道哪些质量维度真正重要。
2. **标准由反例驱动**：开局无法写出完整验收标准，但看到失败样本后，可以稳定指出缺陷。
...
在开放式任务中，用户很难提前写出完整目标函数，但可以在看到候选后判断：“它缺少 X、过度强调 Y、忽略了 Z、把 A 当成了 B。”如果这类反馈只停留在自然语言意见里，它很容易被下一轮生成稀释；如果被记录为结构化审计发现，它就能成为下一轮的控制信号。
...
审计工程允许用户从一个不完整的需求开始，例如“做一个金融研究框架”或“把这些材料变成可执行计划”。它不要求开局就把需求扩写成巨大的规格文档。
...
审计记忆保存可复用失败模式。它不只记录成功模板，还记录哪些模式看起来高级却经常失败。负经验能大面积剪掉低价值搜索分支。
...
审计器可能强化错误 rubric。应使用 Audit-of-Audit、对比样本、反例和外部验证修订审计契约。
...
- 一次性轻量改写；
- 局部润色；
- 已经处于 Autoregressive Extraordinary 区间的任务。
