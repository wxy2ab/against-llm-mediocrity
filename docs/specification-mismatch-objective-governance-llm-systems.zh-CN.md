...
规格失配发生在 LLM 系统能够访问、优化、审计或解释的目标，与真正重要的任务效用之间出现偏离时。

系统可能满足 prompt 却让用户失望，通过 rubric 却失败于真实任务，优化 benchmark metric 却违反语义意图，给出可辩护答案却错过决策标准，或遵循局部指令却损害全局目标。

本文把规格失配展开为 LLM 系统价值保存结构理论中的第六类原始失配。在世界到输出的管线中，规格失配位于评价站点：可访问代理目标 \(\tilde{U}\) 与真实任务效用 \(U\) 之间的关系。

中心问题不是系统是否有足够事实、推理、概率质量或局部一致性，而是：**系统究竟在朝什么标准优化、选择、审计、奖励或修订，而这个标准是否保存真正重要的任务价值？**

本文区分规格失配与观测-表征失配、状态失配、拟合边界失配、支持失配和聚合失配，并给出规格失败类型：目标欠规格、代理过拟合、指标捕获、隐性效用损失、作用域漂移、偏好不一致、局部-全局目标冲突、验证器不完整、rubric 脆弱以及 Goodhart 式利用。

本文主张，规格失配在 LLM 系统中特别核心，因为许多开放任务一开始并没有完整规格。真实成功条件常常通过候选失败、边界情况、专家审查、执行反馈、用户纠正和下游后果逐步发现。

建设性响应是 **目标治理**（Objective Governance）：对任务目标进行有纪律的归纳、修订、定界、审计和撤销。目标治理把 rubric、成功条件、偏好规则、评价标准、验收测试、验证器契约和效用假设视为受治理对象，而不是静态 prompt 文本。

它直接连接审计工程：失败不应只是降低分数，而应更新规格、创建控制增量、添加回归护栏，并修订目标适用条件。

在统一的受治理 LLM 架构中，观测治理确保任务相关变量进入表征；状态治理区分潜在体制；路由治理激活正确能力；支持治理让高价值候选可达；组合治理保存全局结构。

目标治理决定什么算成功，并防止整个系统优化错误目标。
...
这是一篇较长的工作稿。前半部分（第 1–8 节）把规格失配确立为一种原始失配，并将它与其余五类失配区分开来。后半部分（第 9–29 节）展开建设性的响应——目标治理，并在若干具体领域中落地。附录收集了可复用的模板与清单。

- [1. 在统一理论中的位置](#1-在统一理论中的位置)
- [2. 核心定义](#2-核心定义)
- [3. 规格不等于指令](#3-规格不等于指令)
- [4. 为什么规格失配是原始失配](#4-为什么规格失配是原始失配)
- [5. 与其他五类失配的边界](#5-与其他五类失配的边界)
- [6. 规格失配类型](#6-规格失配类型)
- [7. 形式模型](#7-形式模型)
- [8. 规格失配作为修复算子的门](#8-规格失配作为修复算子的门)
- [9. 目标治理](#9-目标治理)
- [10. 规格审计](#10-规格审计)
- [11. 规格失配的审计发现](#11-规格失配的审计发现)
- [12. 目标修复的控制增量](#12-目标修复的控制增量)
- [13. 规格失配回归护栏](#13-规格失配回归护栏)
- [14. 目标对象与 GKO 集成](#14-目标对象与-gko-集成)
- [15. 目标治理与 SGAR](#15-目标治理与-sgar)
- [16. Text-to-SQL 中的目标治理](#16-text-to-sql-中的目标治理)
- [17. 代码修复中的目标治理](#17-代码修复中的目标治理)
- [18. 写作与分析中的目标治理](#18-写作与分析中的目标治理)
- [19. Agentic 工作流中的目标治理](#19-agentic-工作流中的目标治理)
- [20. 目标治理协议](#20-目标治理协议)
- [21. 目标治理与人机协作](#21-目标治理与人机协作)
- [22. 目标治理的失败模式](#22-目标治理的失败模式)
- [23. 设计模式](#23-设计模式)
- [24. 最小实现架构](#24-最小实现架构)
- [25. 什么时候需要目标治理](#25-什么时候需要目标治理)
- [26. 与 Goodhart 和机制设计的关系](#26-与-goodhart-和机制设计的关系)
- [27. 与其他治理层的关系](#27-与其他治理层的关系)
- [28. 规格失配的自审计](#28-规格失配的自审计)
- [29. 结论](#29-结论)
- 附录：[A. 紧凑术语表](#appendix-a-紧凑术语表) · [B. 最小目标对象模板](#appendix-b-最小目标对象模板) · [C. 规格审计清单](#appendix-c-规格审计清单) · [D. 示例目标账本条目](#appendix-d-示例目标账本条目)
...
目标治理应接入受治理知识对象（Governed Knowledge Object / GKO）模型。
...
状态治理智能体范式（State-Governed Agent Regime / SGAR）把目标连接到硬状态：目标的改变因此是一次有记录、经校验的事件，而不是一次悄然漂移的上下文更新。
