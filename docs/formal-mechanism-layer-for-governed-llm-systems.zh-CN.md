...
六类原始失配解释了任务价值在哪里发生结构性扭曲：观测-表征、状态、拟合边界、支持、聚合，以及规格失配。它们是 **任务价值的结构性诊断轴**，也是实践中的第一工程入口。

相比之下，形式化机制层回答的是另一个问题：一旦失败已经被观察到，并且已经被对象化为任务特定控制对象，实际的 LLM 系统中究竟是哪类组件机制在解释或放大这一失败？

我们把 LLM 系统建模为一个在部分可观测环境中运行的近似决策系统。系统并不直接拥有真实状态、真实转移函数、真实奖励、完整动作空间或完整观测通道。相反，它通过一组近似组件行动：
...
形式化机制层在观测、状态、验证器、执行轨迹、候选空间和路由边界都较稳定的任务上最可操作。

在开放式创作任务中，如果尚未先把失败转化为治理对象，直接讨论 `dynamics_world_model` 或 `specification_reward` 往往会显得伪精确。应先构造的治理对象通常包括：角色状态机、叙事骨架、伏笔账本、风格守卫、节奏控制器或读者模拟器。对这类任务，应先有六类失配驱动的对象构造，再有机制归因。
...
这也解释了为什么“表征诱导的价值上限”属于机制层。一旦与效用相关的区分在 `B_\theta` 之前被折叠，后续的路由、支持、聚合或搜索都无法可靠恢复它。恢复必须依赖新增的观测或表征修复。

当 `\mathcal{M}_\theta` 的某个组件与任务所要求的状态存在偏差，且这种偏差改变了可达价值时，就发生机制失配。
...
当真实效用、显式任务目标、评估器、奖励代理、验收标准或模型内部判断之间出现偏离，且该偏离足以改变候选排序时，就发生规格 / 奖励失配。
...
搜索 / 执行失配发生在以下情形：正确候选已经位于有效支持之内；相关信息、目标、动作空间、能力与路由都已足够接近正确；但当前的搜索、排序、验证或执行过程仍然未能找到、选中、保留或完成该候选。
...
干预并不是在机械上纯净分离的。单一改动往往同时影响多条机制轴。
