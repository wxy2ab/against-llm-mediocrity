...
许多 LLM 失败并不是因为输出的每个局部部分都很差。它们发生在局部合理、局部有用或局部被改进的部分无法组合成全局有价值的产物时。一个 SQL query 可能有合理的 `SELECT`、看似可行的 `JOIN` 和可辩护的 `WHERE` clause，却返回错误答案。一个代码补丁可能修复局部症状，却破坏隐藏不变量。一个研究综合可能包含准确摘要，却不能支撑中心结论。一个长程 agent 可能完成多个子任务，却没有推进真实项目状态。

本文把 **聚合失配** 展开为 LLM 系统价值保存结构理论中的原始失配之一。聚合失配发生在局部价值信号不能在构造最终产物的组合操作下保存全局任务效用时。它是狭义上可称为 **自回归平庸** 的核心结构形式：当任务依赖非局部依赖、延迟承诺、接口一致性、全局不变量或端到端语义时，局部可能的 continuation、局部 refinement 和局部 critique 往往会产生流畅但全局次优的产物。

聚合失配不同于支持失配、规格失配、状态失配、观测-表征失配或拟合边界失配。即使相关信息存在、状态已知、正确能力已激活、高价值组件可达且目标清晰，它仍可能发生。失败位于 **组合算子**：把 parts、clauses、modules、arguments、decisions、tool outputs 或 agent actions 组装成全局对象的过程。
...
可以在保持其他站点不变的情况下，只改变组合关系来独立测试聚合失配。
...
当这些对象具有作用域、证据、强度和撤销条件时，它们成为 GKO 或 GEO。
