...
**实验配套：** [Artifact-v4 实验证据、理论差距与 Agent 含义](./aggregation-mismatch-v4-claims-theory-gap.zh-CN.md) · [Artifact-v5 稳定编辑 Agent](./aggregation-mismatch-v5-stable-editing-agent.zh-CN.md) · [Artifact-v7 机制恢复与确定性交付](./aggregation-mismatch-v7-mechanism-recovery.zh-CN.md) · [Artifact-v8 运行时所有权与语义寻址](./aggregation-mismatch-v8-runtime-ownership-routing.zh-CN.md) · [Artifact-v9 最小 scaffold 与 verifier receipt](./aggregation-mismatch-v9-minimal-scaffold-recovery.zh-CN.md) · [Artifact-v10 语义合同与 runtime canonicalization](./aggregation-mismatch-v10-semantic-contract-canonicalization.zh-CN.md) · [Artifact-v11 地址漂移与配置交付](./aggregation-mismatch-v11-config-delivery-transfer.zh-CN.md) · [Artifact-v12 漂移剂量与交付尺度路由](./aggregation-mismatch-v12-scale-routing-transfer.zh-CN.md) · [Artifact-v13 状态漂移与 Intent rebase（归档）](./aggregation-mismatch-v13-state-drift-intent-rebase.zh-CN.md) · [Artifact-v14 Post-Compile Drift 与 Exact Recovery](./aggregation-mismatch-v14-post-compile-drift-recovery.zh-CN.md) · [Artifact-v15 Intent 冲突治理](./aggregation-mismatch-v15-intent-conflict-governance.zh-CN.md) · [Artifact-v16 匹配冲突恢复](./aggregation-mismatch-v16-matched-conflict-recovery.zh-CN.md) · [Artifact-v17 Unlock 信息与不可解除升级](./aggregation-mismatch-v17-unlock-info-escalate.zh-CN.md) · [V1–V12 + V14–V17 实验总览](./aggregation-mismatch-v1-v12-v14-v17-experiment-summary.zh-CN.md) · [Patch 与完整重写受控实验](./patch-vs-full-rewrite-controlled-experiment.zh-CN.md)
...
聚合失配是机制无关的局部—全局失败：部署中的局部代理、有限搜索或不可逆提交没有保存全局补全价值。自回归链式分解本身可以精确表示任意联合分布与全局约束，因此不是该失配的充分原因。旧称“自回归平庸”混淆了聚合失败与支持侧的概率质量集中，本文不再把它作为机制术语。
...
真实效用是否能被窗口受限、复杂度受限的局部可加函数逼近，是任务侧的结构属性。可用 `α_k` 表示这种局部可分解性，用 `1-α_k` 表示非局部耦合剂量。因为公式中没有模型、部署评分器、搜索或提交规则，低 `α_k` 只能说明任务会给局部代理施加压力，不能说明某个具体系统已经发生严重聚合失配。

系统侧应在固定候选或决策集上比较其实际局部排序与全局补全价值 `Q*`。可报告 argmax 不一致率、排序相关，或系统实际选择相对于 `Q*` 最优选择的后悔 `Reg_agg`。候选集只包含两系统都已获得的候选时，度量主要定位组合、排序与提交；相对于完整可行集时，它还会吸收支持与搜索失败。实验必须说明采用哪一个条件集。

这个区分解释了为何同一个强耦合任务可以被脚手架修复：任务的 `α_k` 没有变化，变化的是系统可用的全局状态、搜索过程和实际局部代理，因此 `Reg_agg` 下降。反过来，局部可分解任务也可能被错误评分器或提交规则做坏。
...
状态失配问系统在固定表征下形成或更新的信念是否符合证据并支持正确行动排序。
...
这是精确的链式法则，不对可表示的全局依赖施加局部性限制。精确条件分布可以把全局校验、长程绑定与未来约束编码进每个条件项。因此，不能从“自回归”直接推出“局部最可能选择稀释全局价值”。

真正需要比较的是部署时局部代理与全局补全价值。对位置 $k$ 的候选决策 $d$，令

\[
Q_k^\star(d)=\sup_{Y_{>k}}U(Y_{<k},d,Y_{>k};S_{\mathrm{world}}).
\]

若局部得分、解码器或有限搜索使用的 $q_k(d)$ 满足

\[
\arg\max_d q_k(d)\ne\arg\max_d Q_k^\star(d),
\]

并且早期提交缩小了后续可达集合，才出现聚合失配。条件近似误差、贪心或截断解码、搜索不足、未来约束未外部化与不可逆提交都可能造成这一差异。扩散生成、MCTS 选点、人类分段写作或任何逐步组装过程也可能发生同一种失配；精确条件、自洽搜索或充分外部化状态下的自回归生成则可以不发生。

在许多任务中，部署局部代理与全局价值确实高度对齐：
...
在这些任务里，局部 likelihood 可以是局部和全局质量的有用代理。但这是任务、训练产物与解码规则之间的经验对齐，不是自回归分解的定理。

聚合失配常在任务依赖部署过程尚未表示、搜索或验证的非局部性质时显现：
...
现有受控结果也支持这一定位，而不支持把分解方式当成病因。V2 中，同一类模型的直接闭环构造为 `2/45`，而对给定无效候选的审计为 `176/180`；V4 中，在不改变自回归架构的情况下，外部化完整答案 bits 把周期构造从 `13/54` 提升到 `53/54`。这些比较分别受 candidate-conditioned auditing 与 ceiling 等限制，但共同表明：恢复来自可用全局信息、搜索负担与构造—验证分工的改变，而不是“变得不自回归”。详见[生成—验证不对称证据](./aggregation-mismatch-generation-verification-asymmetry-evidence.zh-CN.md)与 [Artifact-v4 理论差距](./aggregation-mismatch-v4-claims-theory-gap.zh-CN.md)。

因此，“自回归平庸”不再作为聚合失配的子类或机制名使用。若当前模型与解码器的概率质量经验性地把生成拉回常见、流畅但低价值的区域，可非正式称为“自回归引力”，但该现象主要由支持失配描述。
...
| 全局效用 | 组装产物的任务级价值。 |
| 自回归引力（经验昵称） | 当前模型与解码器的概率质量回落到常见、流畅但低价值区域的支持侧倾向；不是架构定理。 |
