...
**中文：** [从聚合失配 V1–V12、V14–V17 到 Agent 工程](./aggregation-mismatch-agent-engineering-lessons-v1-v12-v14-v17.zh-CN.md)

**Evidence companion:** [V1–V12 and V14–V17 experiment summary](./aggregation-mismatch-v1-v12-v14-v17-experiment-summary.md)
...
14. Retry requires an authoritative state transition. If lock, version,
    authority, and preconditions are unchanged, wait or escalate instead of
    growing context and repeating the same write.
15. After a valid transition, start with the minimum sufficient receipt and
    verified plan; expand to a target slice or full state only when operation
    semantics require current values.
16. Treat typed escalation as a first-class safe terminal outcome, but never
    count it as commit success or downstream problem resolution.
...
- V17's four information arms are all 24/24: U for information superiority or
  equivalence, but E that receipt-only suffices for this absolute-plan task.
- V17 typed escalation is 24/24 with commit=0: E for the frozen governance
  contract, not evidence that escalation improves task completion.
...
arms. V16 matches two turns across all three conflict arms; Reread uses a
29,589 median tokens but remains 0/24 while runtime Rebase uses 17,439 and is
24/24. Never treat missing timeout
...
turns, keeping first rejection and recovery under one run key. V16 reconstructs
96 episodes from 1,752 events and 168 provider turns; all conflict arms retain
their matched second turn within the original episode.
V17 reconstructs 192 episodes from 3,840 events and 384 provider turns, with
exactly 20 events per episode. Its ledger also proves that every Module A arm
submitted identical recovery operations for all 24 tasks, separating observed
context use from the headline success rate.
...
- V16's machine primary passes, but a stale Pilot `stop_if` remains in the
  frozen manifest; its evidence state is also `share_with_caveats`.
- V17-A1 fails a superiority gate at ceiling; it cannot be relabeled
  equivalence. V17-B1 passes, but compares escalation compliance with commit
  completion, so endpoint identity must remain attached to the result.
...
V16 matches the number of provider turns across three conflict arms. Generic
Retry and Reread Only remain locked and finish 0/24; runtime Unlock + Rebase
finishes 24/24. The production rule is:

```text
retry_allowed
AND authoritative_state_changed
AND preconditions_revalidated
→ one bounded recovery attempt

otherwise
→ reject / wait / escalate
```

V16 identifies this runtime policy bundle, not a pure information effect:
Rebase alone receives recovery authority, unlocked \(S_2\), and the target
slice. Do not infer that current context is unnecessary after a valid transition.

V17 holds the unlock and authoritative \(S_2\) fixed. For its verified
absolute-value plan, RECEIPT, OLDSTATE, TARGET-SLICE, and REBASE-PLAN all reach
24/24 and emit identical operations; OLDSTATE consumes 74.9% more median
tokens than RECEIPT. This supports a **minimum-sufficient-context ladder for
this operation class**, not population equivalence:

```text
typed receipt + verified plan
→ target-state slice if current values affect the operation
→ broader state only if verifier/compiler requests it
```

When unlock is impossible, V17's typed Escalate route reaches 24/24 accepted
terminal outcomes with 0/24 commits. The engineering rule is to make
`ESCALATED_FOR_POLICY` a first-class audited terminal state with a downstream
owner. It must remain separate from both `COMMITTED` and “problem resolved.”
...
Conflict Governor
  → feasibility check → wait / rebase once / replan / typed escalate

Context Broker
  → receipt + verified plan → target slice → broader state, only on demand
...
8. Distinct terminal states for commit, reject, wait, escalate, and rollback.
...
9. Context broker that expands recovery evidence only when operation semantics
   or verifier diagnostics require it.
...
7. Transfer to real code, configuration, database, spreadsheet, and document
   tasks.
8. Track escalation quality, downstream resolution, queue time, and human cost
   separately from commit success.

## 18. Application map

- **Code agents:** symbol/AST plan → compile current edits → seal → hash recheck
  → format/type/test → diff audit → atomic commit; when a symbol is owned by
  another writer, wait or emit a typed escalation instead of repeating apply.
...
- **Multi-agent:** dispatch only ready tasks and share a hard ledger; bind every
  ...
  Every escalation names its conflict owner and downstream resolver.
...
- The same write is repeated against an unchanged lock/version while only
...
- Treating escalation compliance as commit success or resolved business work.
- Dumping the full old state into every recovery prompt without testing whether
  a typed receipt and verified plan are already sufficient.
...
11. On non-ceiling relative, CAS, and cross-object operations, when does a
    target-state slice outperform receipt-only context after the same unlock?
12. Can receipt-only be shown non-inferior on absolute plans under a
    preregistered cost–reliability gate?
13. When agents freely choose among commit, wait, rebase, and escalate, can
    they identify the conflict class without interface-imposed routing?
14. What fraction of typed escalations are eventually resolved, at what human
    cost and delay?
15. Does the V16 same-state retry result replicate on a second model and real
    expiring leases, Git locks, and database locks?
...
Context broker owns minimum-sufficient recovery disclosure
...
> route stale/locked rejection through a bounded conflict governor; disclose
> recovery context on demand; make typed escalation an audited non-commit
> terminal; put every write behind atomic execution, global verification, and
> commit/rollback; condition delivery and scaffold intensity on real cost,
