...
Patch 相对完整重写的专项设计、机制控制、预算敏感性与 agent 接口含义，见独立实验文档：[English](./patch-vs-full-rewrite-controlled-experiment.md) · [中文](./patch-vs-full-rewrite-controlled-experiment.zh-CN.md)。
...
### Artifact-v4 对机制解释的进一步收窄

18 个新 DeepSeek holdout、756/756 个 run key 的 v4 P0 得到：

| 对比 | 实例级差与 95% interval | 允许的解释 |
|---|---:|---|
| 完整 cut-set − 无锚点 | +0.741 [0.574, 0.870] | 足够正确 bits 强恢复 |
| 结构 cut-set − 等量随机正确 bits | −0.019 [−0.056, 0.000] | 不支持结构位置特异收益 |
| compact seed+executor − 无锚点 | +0.148 [0.037, 0.259] | 紧凑可执行状态有正但有限收益 |
| five candidate rewrite − 无候选 | −0.111 [−0.222, −0.019] | 候选本身不保证帮助全文重写 |
| five audit − rewrite | +0.870 [0.722, 0.981] | 操作与输出的组合差异，不是纯验证效应 |

无锚点构造在独立 300/900/1800 秒下为 0.241/0.370/0.463；\(N=24\)
在 1800 秒达到 17/18，\(N=32,48\) 仍为 5/18 与 3/18。自然/逆序条件为
54/54 与 53/54，受 ceiling 限制。

原始 JSONL 中一个 legacy worker shard 的 7 行曾被误记为 `format_invalid`。
