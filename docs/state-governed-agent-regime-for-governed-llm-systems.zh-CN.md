...
- [2. 运行时问题：上下文不是状态](#2-运行时问题上下文不是状态)
- [3. 状态治理智能体范式](#3-状态治理智能体范式)
- [4. 状态权威与状态表面](#4-状态权威与状态表面)
- [5. 上下文降权规则](#5-上下文降权规则)
- [6. 状态模型](#6-状态模型)
- [7. 转移契约](#7-转移契约)
- [8. 验证器分层](#8-验证器分层)
- [9. 状态转移类型](#9-状态转移类型)
- [10. 运行时循环](#10-运行时循环)
- [11. SGAR 与六类原始失配](#11-sgar-与六类原始失配)
- [12. SGAR 与知识治理](#12-sgar-与知识治理)
- [13. SGAR 与审计工程](#13-sgar-与审计工程)
- [14. SGAR 防止的失败模式](#14-sgar-防止的失败模式)
- [15. 权威分离](#15-权威分离)
- [16. 幂等、重放与回滚](#16-幂等重放与回滚)
- [17. 状态不变量](#17-状态不变量)
- [18. 完成作为受治理转移](#18-完成作为受治理转移)
- [19. 记忆写入作为受治理转移](#19-记忆写入作为受治理转移)
- [20. 人类批准与协作状态](#20-人类批准与协作状态)
- [21. 多 Agent 系统中的 SGAR](#21-多-agent-系统中的-sgar)
- [22. Text-to-SQL 中的 SGAR](#22-text-to-sql-中的-sgar)
- [23. 代码 Agent 中的 SGAR](#23-代码-agent-中的-sgar)
- [24. 研究 Agent 中的 SGAR](#24-研究-agent-中的-sgar)
- [25. SGAR 与工具使用](#25-sgar-与工具使用)
- [26. 状态压缩与上下文渲染](#26-状态压缩与上下文渲染)
- [27. 非单调状态与撤销](#27-非单调状态与撤销)
- [28. SGAR 决策规则](#28-sgar-决策规则)
- [29. 最小 SGAR 实现](#29-最小-sgar-实现)
- [30. 理论命题](#30-理论命题)
- [31. 与形式传统的关系](#31-与形式传统的关系)
- [32. SGAR 自身的风险与失败模式](#32-sgar-自身的风险与失败模式)
- [33. 设计原则](#33-设计原则)
- [34. 标准 Schema](#34-标准-schema)
- [35. 结论](#35-结论)
- [附录 A：紧凑术语表](#附录-a紧凑术语表)
- [附录 B：最小 Checklist](#附录-b最小-checklist)

---

## 摘要

大型语言模型（LLM）agent 常被描述为能够随时间规划、记忆、行动、观测、修订和完成任务的系统。实践中，许多这类系统依赖语言模型上下文窗口作为进展的表面载体。上下文会说某个文件已修改、某个测试已运行、某个 bug 已修复、某个用户偏好已记住、某个任务已完成，或某个计划步骤已执行。

但语言上下文可以描述进展，而不授权进展；它可以总结状态，而不是状态；它可以声称完成，而不提交已验证转移。

本文提出 **状态治理智能体范式**（SGAR）：一种面向受治理 LLM 系统的运行时体制，其中进展不是由模型声称发生了什么来定义，而是由外部、可检查、可重放的硬状态中的已提交转移来定义。

SGAR 把上下文窗口视为叙事工作区和提案界面，而不是状态权威来源。只有当 agent 行动通过显式转移契约时，才改变系统权威状态：
...
SGAR 不是新的提示模式，也不是第七类原始失配。它是 LLM 系统价值保存统一理论的运行时层。

六类原始失配诊断价值在世界到输出管线中的丢失位置。知识治理把控制知识外化为受治理对象。审计工程把失败转化为控制增量和回归护栏。SGAR 决定哪些观测、修复、对象更新、工具效果、记忆写入和任务完成真正进入硬状态。
...
**状态治理智能体范式** 是一种运行时架构，其中 agent 进展由外部状态模型上的已提交转移定义。
...
状态治理智能体范式用转移权威取代叙事权威。系统只通过已验证转移改变状态：
