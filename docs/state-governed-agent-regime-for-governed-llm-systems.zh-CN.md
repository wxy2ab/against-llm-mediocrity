...
本文提出 **状态治理智能体范式**（SGAR）：一种面向受治理 LLM 系统的运行时体制，其中进展不是由模型声称发生了什么来定义，而是由外部、可检查、可重放的硬状态中的已提交转移来定义。SGAR 把上下文窗口视为叙事工作区和提案界面，而不是状态权威来源。只有当 agent 行动通过显式转移契约时，才改变系统权威状态：
