...
本文提出一种结构性解释，用于说明这个区间何时以及为何出现。核心主张是：高价值 LLM 系统设计不只是更好生成的问题，而是 **价值保存** 在世界到输出管线中的问题。任务价值必须穿过观测、表征、潜在状态识别、能力路由、策略支持、局部到全局聚合以及规格。失败发生在价值相关结构被丢失、混叠、路由到错误能力、分配到不足的概率质量、被局部优化但全局破坏，或在错误代理下被评估时。我们把这些形式化为六类原始失配：**观测-表征失配**、**状态失配**、**拟合边界失配**、**支持失配**、**聚合失配** 和 **规格失配**。
...
这种结构视角统一了三类系统层干预。**知识治理** 把任务特定控制知识外化并修订为受治理对象。**审计工程** 把失败信号转换为局部控制增量与回归护栏。**状态治理型 Agent 体制（State-Governed Agent Regime / SGAR）** 为长程系统提供硬状态权威，确保计划、观测、验证、修正与撤销成为已提交的状态转移，而不是松散的上下文叙事。它们共同实现一个更广泛的原则：保留局部对齐的模型能力，但把高失配任务组件转化为低失配控制对象，使其可以被审计、修订、撤销和复用。
...
- [8. 知识治理](#8-知识治理)
- [9. 审计工程](#9-审计工程)
- [10. 状态治理型 Agent 体制](#10-状态治理型-agent-体制)
- [11. 统一对象模型](#11-统一对象模型)
...
第二，六类原始失配对应这个管线中的六个结构性不同站点。观测-表征失配关注决定性世界变量是否进入模型可访问表征。状态失配关注相关潜在状态是否能在可用观测下被识别。拟合边界失配关注已学能力是否在正确域内被激活。支持失配关注高价值结构是否在系统策略下有足够概率质量或可达性。聚合失配关注局部改进是否能组合成全局价值。规格失配关注可访问代理目标是否代表真实任务效用。
...
当世界中的任务相关变量在进入系统工作表征之前被丢失、压缩、混叠、省略，或变得不可操作时，就会发生观测-表征失配。
...
且这种近似对策略或控制过程而言不可区分时，就存在观测-表征失配。系统把两个价值不同的情境折叠成同一个操作性表征。
...
当效用依赖某个潜在状态，而该状态无法在当前观测通道或表征下被识别时，就会发生状态失配。

观测-表征失配问的是必要变量是否进入表征。状态失配问的是，在给定表征的情况下，系统能否推断自己处于哪个状态。

令 `H` 是潜在状态空间，并令正确策略依赖 `h ∈ H`。当：

```text
P(h | Z) is ambiguous or misranked
```

并且候选行动的效用在不同可能状态间明显不同：
...
这是 **自回归平庸** 的狭义所在地。自回归生成通过局部合理续写推进。在许多任务中，局部合理性是有用的。但当全局价值依赖非局部依赖、精确协调、延迟承诺、长程一致性或结构不变量时，局部好可能不足，甚至具有误导性。
...
> 对于被建模为世界-观测-表征-路由-支持-聚合-评估管线的 LLM 系统，六类原始失配及其复合相互作用，穷尽了任务价值可能被结构性丢失的原始站点。
...
- 可以通过从提示中移除决定性数据库列来扰动观测-表征，同时保持目标、状态、策略和聚合过程不变。
- 可以通过使两个潜在 regime 在观测上模糊来扰动状态，同时保留相同变量和任务目标。
...
在正向一端，模型概率与任务价值在整个任务中相互强化。这个区间可以称为 **自回归卓越**，或更中性地称为 **正向概率-价值对齐**。
...
知识治理治理控制知识。审计工程治理失败写回。**状态治理型 Agent 体制** 治理状态权威。
...
状态失配与部分可观测性有关。系统经常必须选择是在不确定性下行动、分支策略、询问澄清问题、查询工具，还是获取额外观测。观测-表征失配增加了一个上游问题：系统可能不只是对状态不确定；决定性变量可能从未进入表征。
...
高价值 LLM 系统失败，不只是因为模型生成了不完美文本，而是因为任务价值难以在多阶段管线中被保存。决定性变量可能没有进入表征。潜在状态可能没有被识别。正确能力可能没有激活。高价值结构可能支持很低。局部改进可能无法组合。可访问评估器可能优化错误代理。
...
| 自回归平庸 | 聚合驱动的子情形，其中局部续写无法组合成全局价值。 |
